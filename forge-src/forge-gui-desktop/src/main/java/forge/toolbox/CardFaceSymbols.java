...
public class CardFaceSymbols {
    ...
    public static void drawSymbol(final String imageName, final Graphics g, final int x, final int y) {
        final SkinImage img = MANA_IMAGES.get(imageName);
        if (img == null) {
            return; // unknown symbol (e.g. custom multi-color hybrid) has no image; skip drawing
        }
        FSkin.drawImage(g, img, x, y);
    }
    ...
    public static void drawSymbol(final String imageName, final Graphics g, final int x, final int y, final int size) {
        ...
        final SkinImage img = MANA_IMAGES.get(imageName);
        if (img == null) {
            // unknown symbol (e.g. custom W/U/B/R/G hybrid) has no sprite; draw the symbol text instead
            final java.awt.Color old = g.getColor();
            g.setColor(java.awt.Color.WHITE);
            g.drawString(imageName, x + 2, y + size - 4);
            g.setColor(old);
            return;
        }
        FSkin.drawImage(g, img.resize(imageSize, imageSize),
