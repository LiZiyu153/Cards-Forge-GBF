...
public class CostAdjustment {
    ...
    private  static void applySetCostAbility(final StaticAbility staticAbility, final SpellAbility sa, final ManaCostBeingPaid manaCost) {
        ...
        if (StringUtils.isBlank(amount)) {
            System.err.println("CostAdjustment: SetCost static ability on " + staticAbility.getHostCard().getName()
                    + " is missing the Amount parameter. Correct syntax: S:Mode$ SetCost | Amount$ <N>.");
            return;
        }
        ...
        try {
            ...
            System.err.println("CostAdjustment: SetCost static ability on " + staticAbility.getHostCard().getName()
                    + " has a non-numeric Amount '" + amount + "'.");
            return;
        }
