...
public class AbilityUtils {
    ...
    private static void resolveApiAbility(final SpellAbility sa, final Game game) {
        ...
        // check conditions
        if (sa.metConditions()) {
            if (ApiType.PayCost.equals(sa.getApi())) {
                handlePayCost(sa, game);
                return;
            }
    ...
    /**
     * Handles the DB$ PayCost effect: offers the payer a cost; the SubAbility
     * chain continues only if the cost is paid (UnpaidSubAbility$ runs instead
     * when the player declines).
     */
    private static void handlePayCost(final SpellAbility sa, final Game game) {
        final Card source = sa.getHostCard();

        final Player payer;
        if (sa.hasParam("Defined")) {
            payer = getDefinedPlayers(source, sa.getParam("Defined"), sa).get(0);
        } else {
            payer = sa.getActivatingPlayer();
        }
        if (!payer.isInGame()) {
            return;
        }

        final String costStr = sa.getParam("Cost");
        if (StringUtils.isBlank(costStr)) {
            System.err.println("AbilityUtils: PayCost ability on " + source.getName()
                    + " is missing the Cost parameter. Correct syntax: DB$ PayCost | Cost$ 2 | SubAbility$ DBX");
            resolveSubAbilities(sa, game);
            return;
        }

        final Cost cost = new Cost(costStr, false);
        final boolean paid = payer.getController().payCostToPreventEffect(cost, sa, false, new FCollection<>(payer));

        if (paid) {
            sa.resolve();
            resolveSubAbilities(sa, game);
        } else if (sa.hasParam("UnpaidSubAbility")) {
            SpellAbility unpaid = sa.getAdditionalAbility("UnpaidSubAbility");
            if (unpaid != null) {
                resolveApiAbility(unpaid, game);
            }
        }
    }
