...
public final class AbilityFactory {
    ...
    public static final List<String> additionalAbilityKeys = Lists.newArrayList(
            ...
            "UnpaidSubAbility", // PayCost — runs when the player declines the payment
    ...
    private static AbilitySub getSubAbility(CardState state, String sSub, final IHasSVars sVarHolder) {
        if (sVarHolder.hasSVar(sSub)) {
            SpellAbility ab = AbilityFactory.getAbility(state, sSub, sVarHolder);
            if (ab instanceof AbilitySub) {
                return (AbilitySub) ab;
            }
            // AB$ / SP$ records referenced as a SubAbility (e.g. Charm choices):
            // the old cast crashed with ClassCastException. Rebuild the record as a
            // DB$ sub-ability, dropping the activated-ability cost parts.
            if (ab instanceof AbilityApiBased || ab instanceof SpellApiBased) {
                Map<String, String> mapParams = AbilityFactory.getMapParams(sVarHolder.getSVar(sSub));
                mapParams.remove("Cost");
                mapParams.remove("Tap");
                mapParams.remove("Untap");
                return (AbilitySub) getAbility(AbilityRecordType.SubAbility, ab.getApi(), mapParams, null, state, sVarHolder);
            }
            throw new IllegalAbilityException("SubAbility '" + sSub + "' for: " + state.getName()
                    + " is not a DB$ ability: " + ab.getClass().getSimpleName());
        }
