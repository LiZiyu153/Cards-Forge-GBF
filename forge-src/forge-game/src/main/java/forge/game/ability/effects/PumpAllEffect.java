...
import forge.game.card.perpetual.PerpetualColors;
...
public class PumpAllEffect extends SpellAbilityEffect {
    ...
    private static void applyPumpAll(final SpellAbility sa,
            ...
            final List<String> keywords, final ColorSet colors,
            final List<ZoneType> affectedZones) {
        ...
        for (final Card tgtC : list) {
            ...
            if (colors != null) {
                // AddColor$ — the card gains the listed colors in addition to its other colors
                if (perpetual) {
                    tgtC.addPerpetual(new PerpetualColors(timestamp, colors, false));
                }
                tgtC.addColor(colors, true, timestamp, null);
                redrawPT = true;
            }
            if (redrawPT) {
    ...
    public void resolve(final SpellAbility sa) {
        ...
        if (sa.hasParam("AddColor")) {
            colors = ColorSet.fromNames(sa.getParam("AddColor").split(","));
        }
        ...
        applyPumpAll(sa, list, a, d, keywords, colors, affectedZones);
